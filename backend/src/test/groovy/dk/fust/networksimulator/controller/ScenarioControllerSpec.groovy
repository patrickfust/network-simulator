...
class ScenarioControllerSpec extends Specification {
    ...
    def "create scenario"() {
        given:
        def scenario = [name: "Test Scenario", path: "/test", description: "desc", latencyMs: 100, statusCode: 200, responseBody: "ok", timeoutMs: 500, followRedirect: true]

        when:
        def response = mockMvc.perform(MockMvcRequestBuilders.
            post('/_/api/v1/scenarios')
    ...
    def "update scenario"() {
        given:
        def scenario = [name: "Updated", path: "/update", description: "desc", latencyMs: 50, statusCode: 201, responseBody: "body", timeoutMs: 200, followRedirect: false]

        expect:
        mockMvc.perform(MockMvcRequestBuilders.
                put("/_/api/v1/scenarios/${scenarioId}")
                .contentType("application/json")
                .content(new JsonBuilder(scenario).toString()))
    ...
    def "delete scenario"() {
        expect:
        mockMvc.perform(MockMvcRequestBuilders.
                delete("/_/api/v1/scenarios/${scenarioId}")
                .contentType("application/json"))
